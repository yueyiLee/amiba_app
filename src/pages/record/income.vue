...
import { INCOME_TYPES, type ITransactionForm } from '@/api/types/transaction'
...
async function onSubmit() {
  ...
  if (!form.type) {
    uni.showToast({ title: '请选择收入类型', icon: 'none' })
    return
  }
  if (saving.value)
    return
  saving.value = true
  try {
    await createTransaction({ ...form }, 'income')
    uni.showToast({ title: '保存成功：收入已计入', icon: 'success' })
    ...
    <view class="px-[32rpx] pb-[200rpx]">
      ...
      <view class="mb-[28rpx]">
        <view class="text-[26rpx] font-semibold text-[#1F2329] mb-[20rpx]">
          收入类型 <text class="text-[#E5484D]">*</text>
        </view>
        <view class="flex flex-wrap">
          <view
            v-for="t in INCOME_TYPES"
            :key="t"
            ...
            :class="form.type === t ? 'bg-[#FDECEC] text-[#E5484D] border border-[#E5484D]' : 'bg-white text-[#6B7280] border border-[#E5E7EB]'"
            @click="form.type = t"
          >
            {{ t }}
          </view>
      ...
      <!-- 客户 -->
      <view class="mb-[28rpx]">
        <view class="text-[26rpx] font-semibold text-[#1F2329] mb-[12rpx]">客户</view>
        <view
        ...
        >
          ...
          >{{ form.counterparty || '选择客户（选填）' }}</text>
      ...
      <view class="mt-[40rpx]">
        <view
          class="h-[100rpx] rounded-[24rpx] flex items-center justify-center text-[32rpx] text-white font-bold shadow-[0_16rpx_40rpx_rgba(229,72,77,0.28)]"
          :class="saving ? 'bg-[#f09c9e]' : 'bg-[#E5484D]'"
